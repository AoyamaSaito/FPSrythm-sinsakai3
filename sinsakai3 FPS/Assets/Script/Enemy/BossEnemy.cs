...
public class BossEnemy : EnemyBase
{
    ...
    IEnumerator AttackCor()
    {
        while(isAttack)
        {
            yield return new WaitForSeconds((float)shootingPlayer.Rythm * attackTime);
            ...
            switch(currentAttackPatern)
            {
                case 0: //遅い球を4回生成する
                    ...
                    yield return new WaitForSeconds((float)shootingPlayer.Rythm * 2);
                    Instantiate(lateBullet, muzzle.position, Quaternion.identity);
                    yield return new WaitForSeconds((float)shootingPlayer.Rythm * 2);
                    Instantiate(lateBullet, muzzle.position, Quaternion.identity);
                    yield return new WaitForSeconds((float)shootingPlayer.Rythm * 2);
    ...
    IEnumerator NormalAttackCor()
    {
        while (isAttack)
        {
            yield return new WaitForSeconds((float)shootingPlayer.Rythm);
